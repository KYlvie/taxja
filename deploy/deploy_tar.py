#!/usr/bin/env python3
"""Deploy Taxja to new server using tar archive for speed."""
import paramiko
import subprocess
import os
import sys
...
print("=" * 60)
print("STEP 1: Create tar archive locally")
print("=" * 60)

workspace = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
tar_path = os.path.join(workspace, "deploy", "taxja-deploy.tar.gz")

# Use tar command to create archive with exclusions
# On Windows with bash shell
excludes = [
...
print(f"  Creating tar from {workspace}...")
tar_cmd = ["tar", "czf", tar_path] + excludes + [
    "-C", workspace,
...
if result.returncode != 0:
    ...
    # Try with python tarfile as fallback
    ...
    skip_patterns = [
        ...
        '.kiro', 'output', 'tmp', 'models', 'k8s', '.github', 'docs', 'src', 'deploy'
    ]
...
print("\n" + "=" * 60)
print("STEP 2: Upload archive to server")
print("=" * 60)
...
print(f"  Uploading {tar_size:.1f} MB archive...")
start = time.time()
...
print("\n" + "=" * 60)
print("STEP 5: Build frontend on server")
print("=" * 60)
print("  npm install (may take a few minutes)...")
code, _, _ = run_cmd(client,
...
code, _, _ = run_cmd(client,
    ...
    timeout=300)
run_cmd(client, f"ls -la {REMOTE_BASE}/frontend/dist/index.html 2>/dev/null && echo 'Frontend OK' || echo 'Frontend FAILED'")

# Step 6: Start services
print("\n" + "=" * 60)
print("STEP 6: Start all services")
print("=" * 60)
print("  docker compose up --build (several minutes)...")
run_cmd(client,
    f"cd {REMOTE_BASE} && docker compose -f docker-compose.server.yml --env-file .env.prod up -d --build 2>&1 | tail -15",
    timeout=900)

print("  Waiting 25s for services...")
time.sleep(25)
...
print("Next steps: SSL cert + DNS update")
print("=" * 60)
