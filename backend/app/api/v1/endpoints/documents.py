...
def get_document_related_data(
    ...
    **Returns:**
    ...
    def _add_related_transactions(transactions):
        for txn in transactions or []:
            if not txn:
                continue
            collected_transactions[txn.id] = {
                "id": txn.id,
                "description": txn.description,
                "amount": float(txn.amount),
    ...
    if document.document_type == DocumentType.RENTAL_CONTRACT:
        ...
        if linked_property:
            ...
            related_data["property"] = {
                ...
                "id": str(linked_property.id),

                "address": linked_property.address,

                "purchase_price": float(linked_property.purchase_price) if linked_property.purchase_price is not None else None,

                "purchase_date": linked_property.purchase_date.isoformat() if linked_property.purchase_date else None,
            ...
            if recurrings:
                ...
                _add_related_transactions(generated_txns)
                if generated_txns:
                    related_data["generated_transactions_count"] = len(generated_txns)
                ...
                _add_related_transactions(generated_txns)
                generated_count = len(generated_txns)
    ...
    if linked_txns:
        ...
        _add_related_transactions(linked_txns)
    ...
    if document.transaction_id:
        ...
        if txn and txn not in linked_txns:
            ...
            _add_related_transactions([txn])

    if collected_transactions:
        related_data["transactions"] = sorted(
            collected_transactions.values(),
...
def confirm_recurring_from_ocr(
    ...
    if suggestion.get("status") == "confirmed" and suggestion.get("recurring_id"):
        ...
        try:
            ...
            if recurring.property_id is not None:
                linked_property = db.query(Property).filter(
                    Property.id == recurring.property_id,
                ).first()
            ...
            if recurring.property_id is not None and isinstance(ocr_result, dict):
                import json as _json

                updated_ocr = _json.loads(_json.dumps(ocr_result))
                updated_ocr.setdefault("import_suggestion", suggestion)
                updated_ocr["import_suggestion"].setdefault("data", {})
                updated_ocr["import_suggestion"]["data"]["matched_property_id"] = str(recurring.property_id)
                updated_ocr["import_suggestion"]["data"]["no_property_match"] = False
                document.ocr_result = updated_ocr
                flag_modified(document, "ocr_result")
                db.commit()

            property_auto_created = bool(
                linked_property
                and linked_property.purchase_price is not None
                and Decimal(str(linked_property.purchase_price)) <= Decimal("0.01")
            )

            return {
                "message": get_error_message("recurring_income_updated", _get_lang(request, current_user)),
                "recurring_id": recurring_id,
                "property_id": str(recurring.property_id) if recurring.property_id else None,
                "property_address": getattr(linked_property, "address", None),
