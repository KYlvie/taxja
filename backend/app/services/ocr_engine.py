...
class OCREngine:
    ...
    _VISION_EXTRACT_INSTRUCTIONS = (
        ...
        "svs_subtype (REQUIRED: one of: vorschreibung, nachforderung, gutschrift, "
        "saeumniszuschlag, kontoauszug, herabsetzung, versicherungspflicht, "
        "mindestbeitrag, zahlungserinnerung, ratenzahlung, befreiung, kontobestaetigung), "
        "beitragsjahr (the year the Nachbemessung covers, only for nachforderung/gutschrift), "
        "ratenanzahl (number of installments, only for ratenzahlung), "
        "ratenbetrag (monthly installment amount, only for ratenzahlung)\n"
    ...
    _UNIFIED_VISION_EXTRACT_INSTRUCTIONS = (
        ...
        "svs_subtype (REQUIRED: vorschreibung/nachforderung/gutschrift/saeumniszuschlag/"
        "kontoauszug/herabsetzung/versicherungspflicht/mindestbeitrag/zahlungserinnerung/"
        "ratenzahlung/befreiung/kontobestaetigung), "
        "beitragsjahr (year the Nachbemessung covers, for nachforderung/gutschrift only), "
        "ratenanzahl, ratenbetrag (for ratenzahlung only).\n"
        "For Versicherungsbestätigung (insurance confirmation/policy):\n"
        "  insurer_name (CRITICAL: the INSURANCE COMPANY name from the letterhead/logo — "
        "e.g. UNIQA, Generali, Allianz, Wiener Städtische, GRAWE, Zürich, etc. "
        "Do NOT use any other company name that appears elsewhere on the page like a store or employer),\n"
        "  praemie (CRITICAL: extract the ANNUAL total premium = Jahresprämie/Gesamtprämie/Jahresbetrag. "
        "If only a monthly/quarterly/semi-annual amount is shown, MULTIPLY to get the annual total. "
        "If multiple amounts are shown, use the LARGEST annual total. "
        "German format 1.662,36 must become 1662.36 — do NOT return 1.66),\n"
        "  polizze_nr (REQUIRED: the policy number / Polizzennummer / Polizze Nr. / Versicherungsnummer — "
        "this is a unique alphanumeric identifier for the insurance contract, NOT null),\n"
        "  versicherungsart (type of insurance),\n"
        "  date (YYYY-MM-DD),\n"
        "  vertragsbeginn (contract start date YYYY-MM-DD),\n"
        "  zahlungsfrequenz (REQUIRED: extract Zahlungsweise/Zahlungsfrequenz exactly as stated — "
        "one of: monatlich/vierteljährlich/halbjährlich/jährlich),\n"
        "  insurance_subtype (REQUIRED: berufshaftpflicht/betriebsunterbrechung/rechtsschutz/"
        "kfz/gebaeudeversicherung/haushaltsversicherung/private_krankenversicherung/"
        "unfallversicherung/lebensversicherung/other).\n"
