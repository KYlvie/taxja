...
class DocumentPipelineOrchestrator:
    ...
    _AI_TYPE_TO_DB_TYPE = {
        ...
        "versicherung_kuendigung": DBDocumentType.VERSICHERUNGSBESTAETIGUNG,
        "versicherung_praemienaenderung": DBDocumentType.VERSICHERUNGSBESTAETIGUNG,
        "versicherung_jahresbestaetigung": DBDocumentType.VERSICHERUNGSBESTAETIGUNG,
        ...
        *,
        ...
        if action == ProcessingAction.TAX_FORM_IMPORT:
            ...
            if db_type == DBDocumentType.SVS_NOTICE:
                ...
                # Infer subtype from AI classification if VLM didn't provide it
                if not svs_sub:
                    ai_first = doc_ocr.get("_ai_first", {})
                    ai_type = ai_first.get("document_type", "")
                    if ai_type == "svs_vorschreibung":
                        svs_sub = "vorschreibung"
                    elif ai_type == "svs_nachbemessung":
                        svs_sub = "nachforderung"
                    elif ai_type == "svs_jahresbestaetigung":
                        svs_sub = "jahresbestaetigung"
                    if svs_sub:
    ...
    def _validate_asset_classification(self, asset_id, document):
        ...
        try:
            ...
            if netto > 0 and netto <= 1000:
                should_convert = True
                new_category = ExpenseCategory.EQUIPMENT  # GWG: one-time expense, not depreciation
