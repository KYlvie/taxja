...
var ExceptionCode, CapacitorException, getPlatformId, createCapacitor, initCapacitorGlobal, Capacitor, registerPlugin, WebPlugin, encode3, decode, CapacitorCookiesPluginWeb, CapacitorCookies, readBlobAsBase64, normalizeHttpHeaders, buildUrlParams, buildRequestInit, CapacitorHttpPluginWeb, CapacitorHttp, SystemBarsStyle, SystemBarType, SystemBarsPluginWeb, SystemBars;
var init_dist = __esm({
  "node_modules/@capacitor/core/dist/index.js"() {
...
// src/mobile/runtime.ts
var isNativeApp = () => Capacitor.isNativePlatform();
var normalizeNativeApiBaseUrl = (url) => {
...
var getApiBaseUrl = () => {
  ...
  if (isNativeApp()) {
    const nativeBaseUrl = (import.meta.env.VITE_MOBILE_API_BASE_URL || configuredBaseUrl || "https://api.taxja.at/api/v1").trim();
...
// src/utils/locale.ts
var supportedLanguages = ["de", "en", "zh"];
var normalizeLanguage = (language) => {
  const baseLanguage = language?.split("-")[0]?.toLowerCase();
  return supportedLanguages.includes(baseLanguage) ? baseLanguage : "de";
};
    ...
    if (error.response) {
      ...
      switch (error.response.status) {
        case 401:
          ...
          break;
        case 403: {
          const detailObj = error.response.data?._detail_obj;
        ...
        case 429: {
          const quotaObj = error.response.data?._detail_obj;
...
var documentService = {
  ...
  uploadDocument: async (file, onProgress, propertyId) => {
    const formData = new FormData();
    formData.append("file", file);
    const url = propertyId ? `/documents/upload?property_id=${encodeURIComponent(propertyId)}` : "/documents/upload";
    const response = await api_default.post(url, formData, {
      headers: {
        "Content-Type": "multipart/form-data"
      },
      timeout: 12e4,
      // 2min — OCR processing runs synchronously
  ...
  uploadImageGroup: async (files, onProgress, propertyId) => {
    const formData = new FormData();
    files.forEach((file) => {
      formData.append("files", file);
    });
    const url = propertyId ? `/documents/upload-image-group?property_id=${encodeURIComponent(propertyId)}` : "/documents/upload-image-group";
    const response = await api_default.post(url, formData, {
      headers: {
        "Content-Type": "multipart/form-data"
      },
  ...
  batchUpload: async (files, _onProgress) => {
    const formData = new FormData();
    files.forEach((file) => {
      formData.append("files", file);
    });
    const response = await api_default.post(
  ...
  getDocuments: async (filters, page = 1, pageSize = 20) => {
    const params = new URLSearchParams();
    ...
    const response = await api_default.get(`/documents?${params.toString()}`);
    ...
    if (filters?.needs_review) {
      documents = documents.filter((doc) => doc.needs_review);
    }
    return { documents, total: filters?.needs_review ? documents.length : data.total || 0 };
  },
  // Get single document
  getDocument: async (id) => {
    const response = await api_default.get(`/documents/${id}`);
  ...
  getDocumentForReview: async (id) => {
    const docResponse = await api_default.get(`/documents/${id}`);
    ...
    let suggestions = [];
    try {
      const reviewResponse = await api_default.get(`/documents/${id}/review`);
  ...
  // Confirm OCR results
  confirmOCR: async (id) => {
    await api_default.post(`/documents/${id}/confirm`, { confirmed: true });
  },
  // Correct OCR results
  correctOCR: async (id, correctedData) => {
    const response = await api_default.post(
  ...
  downloadDocument: async (id) => {
    const response = await api_default.get(`/documents/${id}/download`, {
      responseType: "blob"
    });
    return response.data;
  },
  // Delete document
  deleteDocument: async (id, deleteMode = "document_only") => {
    await api_default.delete(`/documents/${id}?delete_mode=${deleteMode}`);
  },
  // Get related data for a document (before deletion)
  getDocumentRelatedData: async (id) => {
    const response = await api_default.get(`/documents/${id}/related-data`);
    return response.data;
  },
  // Get document URL for preview
  getDocumentUrl: (id) => {
    const baseURL = api_default.defaults.baseURL || "/api/v1";
    return `${baseURL}/documents/${id}/download`;
  },
  // Confirm property creation from Kaufvertrag OCR suggestion
  confirmProperty: async (id) => {
    const response = await api_default.post(`/documents/${id}/confirm-property`);
    return response.data;
  },
  // Confirm recurring income creation from Mietvertrag OCR suggestion
  confirmRecurring: async (id) => {
    const response = await api_default.post(`/documents/${id}/confirm-recurring`);
    return response.data;
  },
  // Confirm recurring expense creation from invoice/insurance OCR suggestion
  confirmRecurringExpense: async (id) => {
    const response = await api_default.post(`/documents/${id}/confirm-recurring-expense`);
    return response.data;
  },
  // Confirm loan creation from Kreditvertrag OCR suggestion
  confirmLoan: async (id) => {
    const response = await api_default.post(`/documents/${id}/confirm-loan`);
    return response.data;
  },
  // Confirm asset creation from vehicle/equipment Kaufvertrag OCR suggestion
  confirmAsset: async (id, confirmation) => {
    const response = await api_default.post(`/documents/${id}/confirm-asset`, confirmation || {});
    return response.data;
  },
  // Dismiss an import suggestion
  dismissSuggestion: async (id) => {
    const response = await api_default.post(`/documents/${id}/dismiss-suggestion`);
    return response.data;
  },
  // Confirm tax filing data from OCR suggestion (L16, L1, E1a, E1b, etc.)
  confirmTaxData: async (id) => {
    const response = await api_default.post(`/documents/${id}/confirm-tax-data`);
    return response.data;
  },
  // Retry OCR processing (re-process document)
  retryOcr: async (id) => {
    const response = await api_default.post(`/documents/${id}/retry-ocr`, null, { timeout: 12e4 });
    return response.data;
  },
  // Batch import selected bank transactions from Kontoauszug OCR
  confirmBankTransactions: async (id, transactionIndices) => {
    const response = await api_default.post(`/documents/${id}/confirm-bank-transactions`, {
