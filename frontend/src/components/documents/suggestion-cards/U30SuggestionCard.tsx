...
const U30SuggestionCard: React.FC<SuggestionCardProps> = (props) => {
  const { t } = useTranslation();
  const d = props.suggestion.data || {};
  const f = (key: string) => t(`documents.suggestion.fields.${key}`, key.replace(/_/g, ' '));
  return (
    <SuggestionCardShell
      icon="🧾" title={t('documents.suggestion.importU30')}
    ...
    >
      ...
      <div className="suggestion-details">
        {d.meldezeitraum && <Row label={f('meldezeitraum')} value={d.meldezeitraum} />}
...
export default U30SuggestionCard;
